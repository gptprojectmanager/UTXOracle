...
async def test_zmq_connection_status():
    ...
    with patch('zmq.asyncio.Context'):
        ...
        assert listener.is_connected == False, "Listener should not be connected initially"
        
        # Act: Connect
        await listener.connect()
        
        # Assert: Connected after connect()
        assert listener.is_connected == True, "Listener should be connected after connect()"
        
        # Act: Disconnect
        await listener.disconnect()
        
        # Assert: Not connected after disconnect()
        assert listener.is_connected == False, "Listener should not be connected after disconnect()"
...
async def test_zmq_stream_transactions():
    ...
    with patch('zmq.asyncio.Context') as mock_context:
        ...
        for tx in transactions:
            ...
            assert abs(tx.timestamp - start_time) < 2.0, "timestamp must be current time"
...
async def test_zmq_stream_handles_malformed_messages():
    ...
    Requirements:
    - Skip messages that don't have 2 parts (topic + data)
    ...
    with patch('zmq.asyncio.Context') as mock_context:
        ...
        valid_tx = bytes.fromhex("020000000100ffffffff0002400420f000000000080969800000000000000000")
        ...
        assert len(transactions) == 2, "Should yield only 2 valid transactions (skipping 3 invalid)"
