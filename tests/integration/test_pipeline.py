...
async def test_end_to_end_zmq_to_websocket():
    ...
    with patch('zmq.asyncio.Context') as mock_zmq_context:
        ...
        mock_socket.recv_multipart = AsyncMock(return_value=[
            b"rawtx",
            sample_tx_bytes
        ])
        ...
        assert should_process == True, "Transaction should pass UTXOracle filters"
        ...
        assert mock_ws_client.messages_received == 1, "Client should receive 1 broadcast"
        ...
        assert latency_ms < 100, f"Pipeline latency {latency_ms:.2f}ms exceeds 100ms threshold"
        
        # Data integrity check (BTC amounts preserved through pipeline)
        original_amounts = [0.001, 0.002]
        assert processed_tx.amounts == original_amounts, "BTC amounts should be preserved"
        ...
        assert isinstance(processed_tx, ProcessedTransaction), "Module 2 output is ProcessedTransaction"
        assert isinstance(mempool_state, MempoolState), "Module 3 output is MempoolState"
        assert isinstance(ws_message, WebSocketMessage), "Module 4 output is WebSocketMessage"
    ...
    with patch('zmq.asyncio.Context') as mock_zmq_context:
        ...
        for i in range(10):
            tx_bytes = bytes.fromhex(
                "02000000"
                "01"  # 1 input
                + f"{i:02x}" * 32 + "00000000" + "00" + "ffffffff"
                + "02"  # 2 outputs
        ...
        mock_socket = AsyncMock()
        mock_socket.recv_multipart = AsyncMock(side_effect=[
            [b"rawtx", tx] for tx in transactions
        ])
        mock_zmq_context.return_value.socket.return_value = mock_socket
        ...
        async for raw_tx in zmq_listener.stream_transactions():
            ...
            if tx_processor.filter_transaction(parsed):
                processed = tx_processor.to_processed(parsed, timestamp=raw_tx.timestamp)
        ...
        assert mock_ws_client.messages_received == 10, "Should receive 10 WebSocket messages"
        
        # Assert: Final state has reasonable confidence
        # With 10 transactions, confidence should be low but non-zero
        assert 0.0 < final_state.confidence <= 0.3, "Confidence should be in low range with 10 tx"
...
async def test_pipeline_filters_invalid_transactions():
    ...
    with patch('zmq.asyncio.Context') as mock_zmq_context:
        ...
        invalid_tx_1 = bytes.fromhex(
            ...
            + "02" + "a086010000000000" + "00" + "400d030000000000" + "00"
            + "00000000"
        )
        ...
        valid_tx = bytes.fromhex(
            ...
            + "11" * 32 + "00000000" + "00" + "ffffffff"
            + "02"
            + "a086010000000000" + "19" + "76a914" + "00" * 20 + "88ac"
            + "400d030000000000" + "19" + "76a914" + "11" * 20 + "88ac"
            + "00000000"
        )
        ...
        state = mempool_analyzer.get_state()
        assert state.total_received == 1, "Analyzer should only receive valid transactions"
...
def valid_transaction_bytes() -> bytes:
    ...
    return bytes.fromhex(
        ...
        + "11" * 32 + "00000000" + "00" + "ffffffff"
        + "02"  # 2 outputs
        + "a086010000000000" + "19" + "76a914" + "00" * 20 + "88ac"  # 0.001 BTC
        + "400d030000000000" + "19" + "76a914" + "11" * 20 + "88ac"  # 0.002 BTC
        + "00000000"  # locktime
    )
